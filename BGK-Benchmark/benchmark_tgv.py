...
from model import SymCollision
...
NN_Naive_PATH  = 'naive_model.pt'
NN_Sym_PATH  = 'sym_model.pt'
...
naiveModel = load_model(NaiveCollision, NN_Naive_PATH, hidden_size=50, device=device)
symModel = load_model(SymCollision, NN_Sym_PATH, hidden_size=50, device=device)
...
def collide_nn_sym(f_pre):
    ...
    with torch.no_grad():
        out_all = symModel(torch.from_numpy(flat_all).float().to(device))
    ...
    return acc / n_d8
    # --- un‑permute, average, multiply back by ρ ---------------------------
    acc = np.zeros_like(f_pre)
    n_cells = Ly * Lx
    for k, perm in enumerate(d8_perms):
        var_k = out[k*n_cells:(k+1)*n_cells, :].T.reshape(9, Ly, Lx)
        acc  += permute_pop(var_k, np.argsort(perm))   
    phi_post = acc / len(d8_perms)
...
def collide_nn(f_pre):              # shape.f_pre (9, Ly, Lx)
    ...
    with torch.no_grad():
        out = naiveModel(torch.from_numpy(flat).float().to(device))
...
for t in range(n_steps):
    ...
    k = 2.0 * np.pi / Lx
    analytic.append((u0/np.sqrt(2)) * np.exp(-2.0 * nu * k*k * t))
...
print(f"Initial   Naive ⟨|u|⟩ = {avg_naive[0]:.6e},   Sym ⟨|u|⟩ = {avg_sym[0]:.6e},   analytic ⟨|u|⟩ = {analytic[0]:.6e}")
print(f"Final     Naive ⟨|u|⟩ = {avg_naive[-1]:.6e},   Sym ⟨|u|⟩ = {avg_sym[-1]:.6e},   analytic ⟨|u|⟩ = {analytic[-1]:.6e}")
...
plt.semilogy(analytic, '--',    label='Analytic')
